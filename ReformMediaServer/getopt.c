         ...
         int optchr, int err)
{
    if(opterr)
    {
        printf(stderr, "Error in argument %d, char %d: ", oint, optchr+1);
        switch(err)
        {
        case OPTERRCOLON:
            printf(stderr, ": in flags\n");
            break;
        case OPTERRNF:
            printf(stderr, "option not found %c\n", argv[oint][optchr]);
            break;
        case OPTERRARG:
            printf(stderr, "no argument for option %c\n", argv[oint][optchr]);
            break;
        default:
            printf(stderr, "unknown\n");
            break;
        }
    }
